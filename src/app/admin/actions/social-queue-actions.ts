...
export async function createSocialPostDraft(topic: string, country: string) {
    try {
        ...
        const post = await prisma.socialPost.create({
            data: {
                content: captionRes.success ? captionRes.content : 'Error generando texto',
                imageUrl: imageRes.success ? imageRes.url : null, // Uses Unsplash URL if DALL-E fails
                videoPrompt: veoRes.success ? veoRes.content : `Prompt para Veo 3: ${topic} - ${country}`,
