    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <div className="container mx-auto px-4 pt-8 pb-24 max-w-5xl">
                ...
                {/* Tabs de Navegación (Solo si es dueño) */}
                {isOwner && (
                    <div className="flex gap-2 mb-6">
                        <button
                            onClick={() => setActiveTab('vehicles')}
                            className={`px-6 py-3 rounded-xl font-bold transition-all ${activeTab === 'vehicles' ? 'bg-primary-700 text-white shadow-lg' : 'bg-surface border border-surface-highlight text-text-secondary hover:text-text-primary'}`}
                        >
                            Mis Vehículos ({vehiclesToShow.length})
                        </button>
                        <button
                            onClick={() => setActiveTab('reports')}
                            className={`px-6 py-3 rounded-xl font-bold transition-all relative ${activeTab === 'reports' ? 'bg-red-600 text-white shadow-lg' : 'bg-surface border border-surface-highlight text-text-secondary hover:text-text-primary'}`}
                        >
                            Centro de Reportes
                            {user.reportsMade?.filter((r: any) => r.status === 'PENDING').length > 0 && (
                                <span className="absolute -top-1 -right-1 w-3 h-3 bg-white rounded-full border-2 border-red-600 animate-pulse"></span>
                            )}
                        </button>
                    </div>
                )}
